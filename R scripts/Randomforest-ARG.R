...
ggsave(filename = "Predicted_vs_Observed_Methane_Intensity.tiff", width = 8, height = 8, dpi = 300)

ggsave(filename = "Predicted_vs_Observed_Methane_Intensity.pdf", width = 8, height = 8)
...
ggplot(top_20_features, aes(x = reorder(Feature, Importance), y = Importance)) +
  geom_bar(stat = 'identity', width = 0.7, fill = "#AB8CBB") + 
  coord_flip() +
  xlab("Feature") +
  ylab("Importance") +
...
ggsave(filename = "Top_20_Important_Features.tiff", width = 20, height = 8, dpi = 300)
ggsave(filename = "Top_20_Important_Features.svg", width = 20, height = 8)
...
ggsave(filename = "Partial_Dependence_Plots.tiff", width = 20, height = 14, dpi = 300)


ggsave(filename = "Partial_Dependence_Plots.pdf", width = 20, height = 14, device = cairo_pdf)


pd_plot <- partial(best_model, pred.var = "tetracycline__tet.W.N.W.", train = X_train, rug = TRUE) %>% 
  autoplot() + 
  geom_hline(yintercept = mean(y_train), linetype = 2, color = "gray") + 
  theme(panel.border = element_rect(colour = "black", fill = NA),
        panel.background = element_blank())
